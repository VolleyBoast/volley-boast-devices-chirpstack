...
function Decoder(bytes, fport) {
    ...
    if (fport == 1) {
        ...
        decodedData = parseDummpyPayload(bytes);
    }
    else {
...
function addVoboMetadata(decodedData, fport)
{
    ...
    else if(portLastDigit == 3) voboType = "VoBoXB";
    ...
    else if (fport == 199) payloadType = "Dummy Payload";
...
function parseVoboLibGeneralConfigurationPayload(bytes, fport)
{
    ...
    if (decoded.sequenceNumber == 0)
    {
        ...
        if ((fport == 70) || (fport == 71) || (fport == 73)) decoded.lowBattery = parseFloat(((bytes[2] & 0x0F) / 10.0 + 2.5).toFixed(1)); // Low Battery Threshold (4-bit)
...
function parseVoboLibVoboSyncConfigurationPayload(bytes)
{
    ...
    if (decoded.sequenceNumber == 0)
    {
        ...
        decoded.vbsTimeReference = Buffer.from(bytes.slice(3, 7)).readUInt32LE();                       // VoboSync Time Reference (32-bit)
...
function parseVoboXXGeneralConfigurationPayload(bytes)
{
    ...
    if (decoded.sequenceNumber == 1)
    {
        decoded.reserved1 = bytes[1] & 0x03;                                    // Reserved 1 field (2-bit)
        decoded.reserved2 = (bytes[1] >> 2) & 0x03;                             // Reserved 2 field (2-bit)
        decoded.pCntDin3Type = (bytes[1] >> 4) & 0x03;                          // Pulse Count DIN3 Type (2-bit)
        decoded.reserved3 = (bytes[1] >> 6) & 0x03;                             // Reserved 3 field (2-bit)
        decoded.reserved4 = bytes[2] & 0x01;                                    // Reserved 4 field (1-bit)
        decoded.reserved5 = (bytes[2] >> 1) & 0x01;                             // Reserved 5 field (1-bit)
        decoded.pCntDin3Enable = Boolean((bytes[2] >> 2) & 0x01);               // Pulse Count DIN3 Enable (1-bit)
        decoded.reserved6 = (bytes[2] >> 3) & 0x01;                             // Reserved 6 field (1-bit)
        decoded.reserved7 = (bytes[2] >> 4) & 0x01;                             // Reserved 7 field (1-bit)
        decoded.pCntDin3TransmitEnable = Boolean((bytes[2] >> 5) & 0x01);       // Pulse Count DIN3 Transmit Enable (1-bit)
        decoded.reserved8 = (bytes[2] >> 6) & 0x03;                             // Reserved 7 field (2-bit)
        decoded.pCntPeriod = bytes[3] & 0x3F;                                   // Pulse Count Period (6-bit)
        decoded.reserved9 = (bytes[3] >> 6) & 0x03;                             // Reserved 9 field (2-bit)
        decoded.reservedDTC = (bytes[5] << 8) | bytes[4];                       // Reserved DTC Field (16-bit)
    }
...
function parseConfigurationPayload(bytes, fport)
{
    ...
    // VoBo XB Configuration Payloads
    else if(fport == 73 && subgroupID == 4) decoded = parseVoboXBGeneralConfigurationPayload(bytes);
    else if(fport == 73 && subgroupID == 5) decoded = parseVoboXXModbusGeneralConfigurationPayload(bytes);
    else if(fport == 73 && (subgroupID == 6 || subgroupID == 7)) decoded = parseVoboXXModbusGroupsEnableConfigurationPayload(bytes);
    else if(fport == 73 && (subgroupID == 8 || subgroupID == 9 || subgroupID == 10)) decoded = parseVoboXXModbusGroupsConfigurationPayload(bytes);
    else if(fport == 73 && subgroupID == 11) decoded = parseVoboXXModbusPayloadsSlotsConfigurationPayload(bytes);
    else if(fport == 73 && subgroupID == 12) decoded = parseVoboXXEngineeringUnitsConfigurationPayload(bytes);
    else if(fport == 73 && subgroupID == 13) decoded = parseVoboXBHartGeneralConfigurationPayload(bytes);
    else if(fport == 73 && subgroupID == 14) decoded = parseVoboXBHartGroupsConfigurationPayload(bytes);

    return decoded;
}

function parseVoboXBGeneralConfigurationPayload(bytes)
{
    var decoded = {};
    decoded.subgroupID = bytes[0] & 0x0F;                                   // Sub-Group ID (4-bit)
    decoded.sequenceNumber = (bytes[0] & 0xF0) >> 4;                        // Sequence Number (4-bit)
    ...
    if (decoded.sequenceNumber == 0)
    {
        ...
        decoded.powerTime = parseFloat((bytes[2] / 10).toFixed(1));             // Analog Power Time (8-bit)
        decoded.mbEnable = Boolean(bytes[3] & 0x01);                            // Modbus Enable (1-bit)
        decoded.reserved1 = Boolean((bytes[3] >> 1) & 0x01);                    // Reserved 1 field (1-bit)
        decoded.din1TransmitEnable = Boolean((bytes[3] >> 2) & 0x01);           // DIN1 Transmit Enable (1-bit)
        decoded.din2TransmitEnable = Boolean((bytes[3] >> 3) & 0x01);           // DIN2 Transmit Enable (1-bit)
        decoded.din3TransmitEnable = Boolean((bytes[3] >> 4) & 0x01);           // DIN3 Transmit Enable (1-bit)
        decoded.wkupTransmitEnable = Boolean((bytes[3] >> 5) & 0x01);           // WKUP Transmit Enable (1-bit)
    ...
    if (decoded.sequenceNumber == 1)
    {
        decoded.pCntDin1Type = bytes[1] & 0x03;                                 // Pulse Count DIN1 Type (2-bit)
        decoded.pCntDin2Type = (bytes[1] >> 2) & 0x03;                          // Pulse Count DIN2 Type (2-bit)
        decoded.pCntDin3Type = (bytes[1] >> 4) & 0x03;                          // Pulse Count DIN3 Type (2-bit)
        decoded.reserved1 = (bytes[1] >> 6) & 0x03;                             // Reserved 3 field (2-bit)
        decoded.pCntDin1Enable = Boolean(bytes[2] & 0x01);                      // Pulse Count DIN1 Enable (1-bit)
        decoded.pCntDin2Enable = Boolean((bytes[2] >> 1) & 0x01);               // Pulse Count DIN2 Enable (1-bit)
        decoded.pCntDin3Enable = Boolean((bytes[2] >> 2) & 0x01);               // Pulse Count DIN3 Enable (1-bit)
        decoded.pCntDin1TransmitEnable = Boolean((bytes[2] >> 3) & 0x01);       // Pulse Count DIN1 Transmit Enable (1-bit)
        decoded.pCntDin2TransmitEnable = Boolean((bytes[2] >> 4) & 0x01);       // Pulse Count DIN2 Transmit Enable (1-bit)
        decoded.pCntDin3TransmitEnable = Boolean((bytes[2] >> 5) & 0x01);       // Pulse Count DIN3 Transmit Enable (1-bit)
        decoded.vppTransmitEnable = Boolean((bytes[2] >> 6) & 0x01);            // VPP Transmit Enable (1-bit)
        decoded.reserved2 = (bytes[2] >> 7) & 0x01;                             // Reserved 7 field (2-bit)
        decoded.pCntPeriod = bytes[3] & 0x3F;                                   // Pulse Count Period (6-bit)
        decoded.reserved3 = (bytes[3] >> 6) & 0x03;                             // Reserved 9 field (2-bit)
        decoded.reservedDTC = (bytes[5] << 8) | bytes[4];                       // Reserved DTC Field (16-bit)
        decoded.serialPHY = bytes[6] & 0x01;                                    // Serial Physical Layer (1-bit)
        decoded.reserved4 = (bytes[6] >> 1) & 0x07;                             // Reserved 4 Field (3-bit)
        decoded.serialProtoRS485 = (bytes[6] >> 4) & 0x01;                      // Serial Protocol for RS485 (1-bit)
        decoded.reserved5 = (bytes[6] >> 5) & 0x01;                             // Reserved 5 Field (1-bit)
        decoded.serialProtoRS232 = (bytes[6] >> 6) & 0x01;                      // Serial Protocol for RS232 (1-bit)
        decoded.reserved6 = (bytes[6] >> 7) & 0x01;                             // Reserved 6 Field (1-bit)
    }

    return decoded;
}

function parseVoboXBHartGeneralConfigurationPayload(bytes)
{
    var decoded = {};
    decoded.subgroupID = bytes[0] & 0x0F;                                   // Sub-Group ID (4-bit)
    decoded.sequenceNumber = (bytes[0] & 0xF0) >> 4;                        // Sequence Number (4-bit)
    
    decoded.hartAin1Enable = Boolean(bytes[1] & 0x01);                      // HART AIN1 Enable (1-bit)
    decoded.hartAin2Enable = Boolean((bytes[1] >> 1) & 0x01);               // HART AIN2 Enable (1-bit)
    decoded.hartAin3Enable = Boolean((bytes[1] >> 2) & 0x01);               // HART AIN3 Enable (1-bit)
    decoded.hartDiscEnable = Boolean((bytes[1] >> 3) & 0x01);               // HART Discovery Enable (1-bit)
    decoded.hartDiscType = (bytes[1] >> 4) & 0x03;                          // HART Discovery Type (2-bit)
    decoded.reserved1 = (bytes[1] >> 6) & 0x03;                             // Reserved 1 Field (2-bit)
    decoded.hartRspTimeout = bytes[3] << 8 | bytes[2];                      // HART Response Timeout (16-bit)
    decoded.hartMinRspDelay = bytes[5] << 8 | bytes[4];                     // HART Minimum Response Delay (16-bit)
    decoded.reservedHNOM = bytes[6] & 0x03;                                 // Reserved HNOM Field (2-bit)
    decoded.reservedHME = Boolean((bytes[6] >> 2) & 0x01);                  // Reserved HME Field (1-bit)
    decoded.hartDiscTxEnc = bytes[6] >> 3 & 0x1F;                           // HART Discovery Transmit Encoding (5-bit)

    return decoded;
}

function parseVoboXBHartGroupsConfigurationPayload(bytes)
{
    var decoded = {};
    decoded.subgroupID = bytes[0] & 0x0F;                                           // Sub-Group ID (4-bit)
    decoded.sequenceNumber = (bytes[0] & 0xF0) >> 4;                                // Sequence Number (4-bit)
    var groupIdx = decoded.sequenceNumber + 1;                                      // Group index is determined by Sequence Number.
    var groupIdxStr = (groupIdx).toString();
    decoded["hartEnableG" + groupIdxStr] = Boolean(bytes[1] & 0x01);                // HART Group Enable (1-bit)
    decoded["hartAinG" + groupIdxStr] = (bytes[1] >> 1) & 0x03;                     // HART Group AIN (2-bit)
    decoded["hartDevNumG" + groupIdxStr] = (bytes[1] >> 3) & 0x0F;                  // HART Group Device Number (4-bit)
    decoded["hartAddrTypeG" + groupIdxStr] = (bytes[1] >> 7) & 0x01;                // HART Group Address Type (1-bit)
    decoded["hartAddrG" + groupIdxStr] = "";                                        // HART Group Address (6-byte)
    for(let i = 6; i >= 2; i--)
    {
        decoded["hartAddrG" + groupIdxStr] += bytes[i].toString(16).toUpperCase().padStart(2,0);
        if(i != 2) decoded["hartAddrG" + groupIdxStr] += "-";
    }
    decoded["hartTxEncG" + groupIdxStr] = bytes[7] & 0x1F;                          // HART Group Transmit Encoding (5-bit)
    decoded["reserved1"] = (bytes[7] >> 5) & 0x07;                                  // Reserved 1 Field (3-bit)
...
function parseDummpyPayload(bytes)
{
    var decoded = {};

    decoded.dummyData = bytes[0];

    return decoded;
}

function decodeVoboXBHartSensorNumber(sensorNum) {
    const variableTypes = ['LC', 'PV', 'SV', 'TV', 'QV'];    
    // Extract the remainder to determine sensor type (0-4 maps to LC, PV, SV, TV, QV)
    const offset = (sensorNum - 16) % 5;
    const variableType = variableTypes[offset];
    // Extract base sensor number (without type offset)
    const baseSensorNum = sensorNum - offset;    
    const adjustedNum = baseSensorNum - 16;        
    const master = Math.floor(adjustedNum / 80) + 1;
    const deviceRemainder = adjustedNum % 80;    
    // Calculate Device: Device = deviceRemainder / 5 (starts from 0)
    const device = deviceRemainder / 5;
    
    return "HART" + " Master" + master + " Device" + device + " " + variableType;
}
...
function lookupAnalogSensorName(voboType, sensorNum) {
    ...
    [
        ...
        {"Analog Sensor Number":"5","Analog Sensor Name":"DIN1 Pulse Count"},
        {"Analog Sensor Number":"7","Analog Sensor Name":"DIN2 Pulse Count"},     
        {"Analog Sensor Number":"9","Analog Sensor Name":"DIN3 Pulse Count"},
        {"Analog Sensor Number":"14","Analog Sensor Name":"VPP Voltage"},
        {"Analog Sensor Number":"15","Analog Sensor Name":"ADC Temperature"}
    ];
    ...
    else if(voboType == "VoBoXB") {
        analogSensorsTable = analogSensorsTableXB;
    }
    else {
        const errorMsg = voboType + " -- " + "Invalid VoBo Type. Use \"VoBoXX\", \"VoBoTC\" or \"VoBoXP\" or \"VoBoXB\"";
        throw new Error(errorMsg);
    }

    var analogSensorRow = undefined;
    if ((voboType == "VoBoXB") && (sensorNum >=16) && (sensorNum <= 255)) {
        analogSensorRow = {"Analog Sensor Number": sensorNum.toString(), "Analog Sensor Name": decodeVoboXBHartSensorNumber(sensorNum)};
    }
    else {
        analogSensorRow = analogSensorsTable.find(element => element["Analog Sensor Number"] == sensorNum.toString());
    }
...
function lookupDigitalSensorName(voboType, sensorNum) {
    ...
    if((voboType == "VoBoXX") || (voboType == "VoBoXB")) {
...
function lookupUnits(outputType, unitCode) {
    ...
    [
        ...
        {"Units Code":"172","Description":"milliseconds","Abbreviated Units":"ms"},
    ...
    if(outputType == 0) {
        ...
        const errorMsg = outputType + " -- " + "Invalid Output Type. Use 0 for Description or 1 for Abbreviated Units.";
        throw new Error(errorMsg);
    }
...
function printPayload(payload)
{
    ...
    if (payload.data.payloadType == "Analog Input Variable Length")
    {
        ...
        for (let i = 0; i < payload.data.numOfAinPayloads; i++)
        {
            const analogSensorString = payload.data["analogSensorString" + i];
            const engUnitsString = payload.data["engUnitsString" + i];
            const sensorData = payload.data.ainPayloads[i].sensorData0;
    ...
    {
        let dataKeys = Object.keys(payload.data.modbusSlots);
        let stringToPrint = payload.timestamp + " | " + payload.deveui + " | " + payload.data.voboType + " | " + payload.data.payloadType + " | ";
        for (let idx = 0; idx < dataKeys.length; idx++)
        {
            let fieldName = dataKeys[idx];
    ...
    if(payload.data.payloadType == "Dummy Payload")
    {
        let dummyDataHexStr = "0x" + payload.data["dummyData"].toString(16).toUpperCase().padStart(2,0);
        console.log("%s | %s | %s | %s |\n", 
                    payload.timestamp, payload.deveui, payload.data.payloadType, dummyDataHexStr);
        return;
    }
